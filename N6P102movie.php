<html>
	<head>
		<?php
			...
			{
				header("Location: N6P102movie.php?action=a", TRUE, 301);				
			}
			
			if ($_GET['action'] == 'edit')				
			{
				echo '<title>' . ucfirst($_GET['action']) . 'Movie</title>';		
				//retrieve the record's information 
				
				$query = 'SELECT movie_name, movie_type, movie_year, movie_leadactor, movie_director,movie_release, movie_rating FROM movie WHERE movie_id = ' . $_GET['id'];
	...
	<body> 
		<?php
			...
			if (isset($_GET['action']) && $_GET['action'] != '') 
			{
				echo '<form action="N6P103commit.php?action=' . $_GET['action'] . '>&type=movie" method="post">';			
			}
		?>  
			<table>
				<tr>
					...
					<td>
						<?php 
							echo '<input type="text" name="movie_name" value="' . $movie_name . '"/>';
						?> 			
					</td>
				</tr>
				<tr>
